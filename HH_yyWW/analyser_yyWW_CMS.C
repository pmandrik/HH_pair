...
void analyser_yyWW_CMS( string delphes_file, string lhe_file, bool lhe_format = false, string mode = ""){
  ...
  for(int i = 0; i < delphes_files.size(); i++){
    ...
    for(;entry < entrys;entry++){
      ...
      for(int i = 0; i < reader->Photon_; i++){
        ...
        if( TMath::Abs(reader->Photon_Eta[i]) > 2.5 ) continue; // 2.37 ATLAS
        if( TMath::Abs(reader->Photon_Eta[i]) > 1.44 and TMath::Abs(reader->Photon_Eta[i]) < 1.57 ) continue; // 1.37 1.52 ATLAS
      ...
      for(int i = 0; i < reader->Muon_; i++){
        ...
        if( TMath::Abs(reader->Muon_Eta[i]) > 2.4 ) continue; // 2.7 ATLAS
      ...
      for(int i = 0; i < reader->Electron_; i++){
        ...
        if( TMath::Abs(reader->Electron_Eta[i]) > 2.5 ) continue; // 2.47 ATLAS
        if( TMath::Abs(reader->Electron_Eta[i]) > 1.479 and TMath::Abs(reader->Electron_Eta[i]) < 1.566 ) continue; // 1.37 1.52 ATLAS
      ...
      for(int i = 0; i < reader->Jet_; i++){
        ...
        if( TMath::Abs(reader->Jet_Eta[i]) > 2.4 ) continue; // 2.5 ATLAS
...
/*
  Acceptance × efficiency ATLAS (HH -> yyWW, WW -> jjelel)  = 0.085
  
  Acceptance × efficiency Delphes = 0.000916
  In Pythia : P(WW->jjelel) = 2*P(W->jj)*P(W -> elel) / (2*P(W->jj)*P(W -> elel) + P(W->jj)*P(W->jj) + P(W -> elel)*P(W -> elel))
              2 * (0.1075 + 0.1057) * 0.6760 = 0.2882464
              P(HH->yyWW) = 2*P(H->yy)*P(H->WW) / (2*P(H->yy)*P(H->WW) + P(H->yy)*P(H->yy) + P(H->WW)*P(H->WW)) = 0.0208004877314935
  Fraction of HH->yyWW, WW->jjelnu events = 1. * 0.2882464 * 0.0208004877314935 = 0.005995665706847168
  From 500k samples 0.005995665706847168 * 500 * 1000 = 2997.832853423584 events
  Expected number of selected events ~ 0.085 * 2997.832853423584 = 254.81579254100464
  Observed number of selected events = 151, observed Acceptance × efficiency = 0.050
  Systematic uncertantie due to finite MC sample = 12.28 events
*/
